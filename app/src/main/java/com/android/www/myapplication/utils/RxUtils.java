...
import com.android.www.myapplication.network.JsonConvert;
...
//   111111
    public static <T> Observable<Response<T>> requestlist(HttpMethod method, String url, Type type, HttpParams params) {
        return requestlist(method,url,type,null,params,null);

    }

    public static <T> Observable<Response<T>> requestWithCache(HttpMethod method, String url, Type type, HttpParams params, String cacheKey) {
        return requestWithRxCache(method, url, type, null, params, null, cacheKey);
    }
     ...
     * 这个封装其实没有必要，只是有些人喜欢这么干，我就多此一举写出来了。。
     */
    public static <T> Observable<Response<T>> requestlist(HttpMethod method, String url, Type type, Class<T> clazz,
...
//                .cacheMode(CacheMode.FIRST_CACHE_THEN_REQUEST);
        //  return request.adapt(new ObservableBody<T>());
        return request.adapt(new ObservableResponse<T>());
    }
    /**
     * 这个封装其实没有必要，只是有些人喜欢这么干，我就多此一举写出来了。。
     * 这个封装其实没有必要，只是有些人喜欢这么干，我就多此一举写出来了。。
     * 这个封装其实没有必要，只是有些人喜欢这么干，我就多此一举写出来了。。
     */
    public static <T> Observable<Response<T>> requestWithRxCache(HttpMethod method, String url, Type type, Class<T> clazz,
                                                                 HttpParams params, HttpHeaders headers
            , String cacheKey) {
        Request<T, ? extends Request> request;
        if (method == HttpMethod.GET) request = OkGo.get(url);
